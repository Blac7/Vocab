...
import { actionFetchAll, actionsGetWord, actionsHandleError } from '../redux/Actions'
import ABC from './Modal'
...
const Words = () => {
    ...
    const getAllWords = async () => {
        const url = `${base_url}/words/all`
        await axios.get(url)
            .then(res => {
                dispatch(actionFetchAll(res.data))
                setLoading(false)
                setDelLoading(false)
            })
    ...
    const delWord = async (name) => {
        setDelLoading(true)
        const url = `${base_url}/words/delete/${name}`
    ...
    const displayWord = () => (
        <>
        <AddButton getAllWords={getAllWords} />
        {delLoading && ( 
            <div className="del-loading">
                Please Wait. Removing word from List.
            </div>
         )}
                ...
                {words && words.map(w => (
                    <div className="word" key={w._id}>
                        ...
                        <div className="word-btn">
                            ...
                            <button className="btn btn-del" type="button" onClick={() => delWord(w.wordName)}>Remove</button>
                        </div>
                    </div>
                ))}
